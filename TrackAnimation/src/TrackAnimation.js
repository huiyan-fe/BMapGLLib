...
(function () {
  ...
  TrackAnimation.prototype._getZoom = function () {
    return Math.min(
      this._overallView.zoom + DELTA_ZOOM,
  ...
  TrackAnimation.prototype._updateViewAni = function () {
    ...
    var keyFrames = [];
    var duration = this._opts.overallView
      ? this._opts.duration + 2000
      : this._opts.duration;
    ...
    if (this._opts.overallView) {
      keyFrames.push({
        center: new BMapGL.Point(
          this._overallView.center.lng,
          this._overallView.center.lat
        ),
  ...
  TrackAnimation.prototype._addPath = function (path) {
    ...
    var percents = [0];
    for (var i = 1; i < length; i++) {
      var percent = distances[i - 1] / totalDistance;
      // percents.push(percent);
      percents[i] = percents[i - 1] + parseFloat(percent, 10);
      expandPath = expandPath.concat(
        this._getPath(path[i - 1], path[i], percent * TOTAL_NUM)
      );
    }
